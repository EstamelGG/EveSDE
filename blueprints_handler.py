    ...
    try:
        ...
        for blueprint_id, blueprint_data in yaml_data.items():
            try:
                ...
                cursor.execute(
                    'INSERT OR REPLACE INTO blueprint_process_time (blueprintTypeID, blueprintTypeIcon, manufacturing_time, research_material_time, research_time_time, copying_time, invention_time) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    (blueprint_type_id, blueprint_type_icon, times['manufacturing_time'], times['research_material_time'], times['research_time_time'], times['copying_time'], times['invention_time'])
                )
                    ...
                    if 'materials' in mfg:
                        for material in mfg['materials']:
                            if "typeID" in material:
                                ...
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_manufacturing_materials (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, quantity) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, material.get("quantity", -1))
                                )
                    # 处理产出
                    if 'products' in mfg:
                        for product in mfg['products']:
                            if "typeID" in product:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_manufacturing_output (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, quantity) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, product.get("quantity", -1))
                                )
                    # 处理技能
                    if 'skills' in mfg:
                        for skill in mfg['skills']:
                            if "typeID" in skill:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_manufacturing_skills (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, level) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, skill.get("level", -1))
                                )
                    ...
                    if 'materials' in rm:
                        for material in rm['materials']:
                            if "typeID" in material:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_research_material_materials (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, quantity) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, material.get("quantity", -1))
                                )
                    # 处理技能
                    if 'skills' in rm:
                        for skill in rm['skills']:
                            if "typeID" in skill:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_research_material_skills (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, level) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, skill.get("level", -1))
                                )
                    ...
                    if 'materials' in rt:
                        for material in rt['materials']:
                            if "typeID" in material:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_research_time_materials (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, quantity) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, material.get("quantity", -1))
                                )
                    # 处理技能
                    if 'skills' in rt:
                        for skill in rt['skills']:
                            if "typeID" in skill:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_research_time_skills (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, level) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, skill.get("level", -1))
                                )
                    ...
                    if 'materials' in cp:
                        for material in cp['materials']:
                            if "typeID" in material:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_copying_materials (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, quantity) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, material.get("quantity", -1))
                                )
                    # 处理技能
                    if 'skills' in cp:
                        for skill in cp['skills']:
                            if "typeID" in skill:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_copying_skills (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, level) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, skill.get("level", -1))
                                )
                    ...
                    if 'materials' in inv:
                        for material in inv['materials']:
                            if "typeID" in material:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_invention_materials (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, quantity) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, material.get("quantity", -1))
                                )
                    # 处理蓝图发明产出
                    if 'products' in inv:
                        for product in inv['products']:
                            if "typeID" in product:
                                ...
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_invention_products (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, quantity, probability) VALUES (?, ?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, product.get("quantity", -1), product.get("probability", 0))
                                )
                    # 处理技能
                    if 'skills' in inv:
                        for skill in inv['skills']:
                            if "typeID" in skill:
                                ...
                                type_icon = get_type_icon(cursor, type_id)
                                cursor.execute(
                                    'INSERT OR REPLACE INTO blueprint_invention_skills (blueprintTypeID, blueprintTypeIcon, typeID, typeIcon, level) VALUES (?, ?, ?, ?, ?)',
                                    (blueprint_type_id, blueprint_type_icon, type_id, type_icon, skill.get("level", -1))
                                )
